...
# Absolute Value target function
def target_func(x):
    return abs(x)
...
jko_steps = 60        # Number of JKO iterations
inner_iters = 80      # Number of optimizer iterations inside each JKO step
tau = 1           # JKO time step
...
fig.suptitle(r"ReLU network - Target : $y(x) = |x|$", fontsize=16)
...
line_net, = ax_2d.plot(x_np, history_out[0], 'r-', label=r'Network $\Phi_\mu$', linewidth=2)
...
ani.save("jko_evolution_abs.gif", writer='pillow', fps=15, dpi=150)
print("File jko_evolution_abs.gif saved with success!")
